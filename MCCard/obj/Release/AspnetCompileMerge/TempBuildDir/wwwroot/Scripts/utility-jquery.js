...
function checkInput(Cls) {
    ...
    if (!check) {
        var message = '';
        if (minLength > 0) message = '您輸入的格式錯誤\r\n輸入的長度請在' + minLength + '~' + maxLength + '個字範圍內';
        else message = '您輸入的格式錯誤\r\n輸入的長度請在' + maxLength + '個字範圍內';
...
$(document).ready(function () {
    if ($("#background").length > 0 && $("#messageBox").length > 0) {
        ...
        $(window).resize(function () { msgBoxPosition(); });
    }
});
